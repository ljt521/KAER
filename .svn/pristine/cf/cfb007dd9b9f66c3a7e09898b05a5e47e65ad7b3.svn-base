  ...
  <div class="co-top" >
    <swipeout>
      <swipeout-item transition-mode="follow" class="fo-tex" v-for="(academicJoin,index) in academicJoinData" :key="index" >
        <div slot="right-menu" >
          <swipeout-button type="primary" @click.native="onButtonClick(academicJoin)">编辑</swipeout-button>
          <swipeout-button type="warn" @click.native="deleteData(academicJoin.id)">删除</swipeout-button>
        </div>
        <div slot="content" :class="{'vux-1px-b': index !== academicJoinData.length}" >
            ...
            <p style="margin: 10px 0">
              <span class="right">
                ...
                {{academicJoin.academicActivityType}}
              </span>
            </p>
            <p style="margin: 10px 0">
